...
Stop words can be discarded by `stopwords()` in the package **tokenizers**. Stemming can be carried out by `wordStem` in the package **SnowballC**. The results are shown below.
...
scaled_cos_dist <- ggplot() +
    ...
    labs(title = "Scaled Cosine Distance Matrix", x = "Coordinate 1", y = "Coordinate 2")

scaled_euc_dist <- ggplot() +
    geom_point(aes(x = euc_dist_fit[, 1], y = euc_dist_fit[, 2])) +
    labs(title = "Scaled Euclidean Distance Matrix", x = "Coordinate 1", y = "Coordinate 2")
